...
public class ObjectHandler {
...
#else
        bool isRawCOMObject = false;
        bool isWrappedCOMObject = false;
        string comTypeAlias = null;
#endif
    ...
    private object callMethod(IKernelLink ml, int callType, TypeRecord t, object obj, string memberName, int[] argTypes, out OutParamRecord[] outParams) {
        ...
        findMethodMatches(targetMethodName, isCtor, argTypes, methods, t.methodNames, matchingMethods, ml.ComplexType, out err);
        ...
        if (pi != null) {
            ...
            if (fi == null) {
                ...
                if (obj != null && !Utils.IsMono && Marshal.IsComObject(obj) && (t.name == "__ComObject" || t.type.IsInterface)) {
                    ...
                    OutParamRecord[] unusedOutParams;
                    return comDispatchHandler.callDispatch(ml, obj, memberName, callType, argTypes, out unusedOutParams);
                } else {
        ...
        for (int i = 0; i < argTypes.Length; i++) {
            ...
            switch (argType) {
                ...
                case Install.ARGTYPE_VECTOR: {
                    ...
                    Type leafObjectType;
                    leafExprType = getLeafExprType(ml, out leafObjectType);
...
//                    // always comes in as int, not the underying type of the enum.
    ...
    private ExpressionType getLeafExprType0(IKernelLink ml, out Type leafObjectType) {
        ...
        string f = ml.GetFunction(out len);
        if (f == "List") {
            for (int i = 0; i < len; i++) {
...
#if UNITY  // yes, not !UNITY as used elsewhere
    [Serializable]
#endif
