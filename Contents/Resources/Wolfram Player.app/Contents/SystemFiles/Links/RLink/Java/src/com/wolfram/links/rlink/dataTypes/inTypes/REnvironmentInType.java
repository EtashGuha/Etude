...
public class REnvironmentInType extends RInTypeImpl {

	public REnvironmentInType() {
		super(new RInAttributesImpl());
	}

	@Override
	public boolean rPut(String rVar, RExecutor exec) {
		// Note: at this time, we hard-code the global environment here,
		// as the only one supported
		return exec.eval(rVar + " <- .GlobalEnv") && super.rPut(rVar, exec);
	}
}
