...
toContentTypeWithEncoding[req_HTTPRequest] := 
    Replace[
        HTTPProperty[req, "RawBody"], {
            rules:$assocPattern :> If[
                FreeQ[rules, _Association|_File, {2, Infinity}],
                toContentTypeWithEncoding[req, "application/x-www-form-urlencoded", None],
                toContentTypeWithEncoding[req, "multipart/form-data", None]
            ],
            _List|_String|_ByteArray :> 
                toContentTypeWithEncoding[req, "text/plain", HTTPProperty[req, CharacterEncoding]],
            _ :>
                toContentTypeWithEncoding[req, None, HTTPProperty[req, CharacterEncoding]]
        }
    ]
...
$httpRules := $httpRules = Dispatch @ Apply[
    ...
    {res:$httpPattern, "Body"} :> 
        Replace[
            HTTPProperty[res, "RawBody"], {
                None|{}|"" -> "",
                content:_ByteArray?ByteArrayQ /; $VersionNumber >= 11.2 :> 
                    ByteArrayToString[
                        content,
                        Replace[
                 ...
                 content:_List|_ByteArray?ByteArrayQ :> checkBodyBytes[
                    content,
                    FromCharacterCode[
                        Normal[content],
                        Replace[
    ...
    {res:$httpPattern, "BodyByteArray"} :> 
        Replace[
            HTTPProperty[res, "RawBody"], {
                ...
                ""|None :> 
                    ByteArray[{}],
                content_ByteArray?ByteArrayQ :> 
                    content,
                content_List :> 
                    Replace[
                        Quiet @ ByteArray[content], 
                        Except[_?ByteArrayQ] :> (Message[HTTPResponse::encfailed]; $Failed)
                    ],
                content_?StringQ :> 
                    If[
                        $VersionNumber >= 11.2,
                        Replace[
                            Quiet @ StringToByteArray[
                                content,
                                Replace[
                                    HTTPProperty[res, CharacterEncoding],
                                    None -> "ISO8859-1"
                        ...
                        Replace[HTTPProperty[res, "BodyBytes"], r:Except[$Failed] :> ByteArray[r]]
