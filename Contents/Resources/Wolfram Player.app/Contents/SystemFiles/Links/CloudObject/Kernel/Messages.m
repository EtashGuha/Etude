...
addMessage[CloudEvaluate::srvfmt, "Wolfram Cloud returned an unexpected response. Please try again later."]
...
addMessage[CloudObjectInformation::dirlvl, "DirectoryLevel is not one of 0, 1, or Infinity."];
addMessage[CloudObject::invuri, "The URI `1` is not valid."];
addMessage[CloudObject::noicon, "No icon named `1` found for `2`."];
addMessage[CloudObject::invactive, "Active value `1` must be either True or False."];
...
addMessage[CloudObject::memorylimit, "Evaluation aborted because memory limit `1` was exceeded."];
addMessage[CloudObject::timelimit, "Evaluation aborted because time limit `1` was exceeded."];
...
addMessage[CloudObject::invpath, "Cloud object name `1` contains either an illegal character or the reserved path .Objects."];
addMessage[CloudObjectNameFormat::una, "Unable to apply the specified name format `1`."];
addMessage[CloudObjectNameFormat::inv, "CloudObjectNameFormat `1` is not one of \"UUID\", \"UserURLBase\", \"CloudUserID\", \"CloudUserUUID\" or Automatic."]
addMessage[$CloudObjectNameFormat::inv, "$CloudObjectNameFormat `1` is not one of \"UUID\", \"UserURLBase\", \"CloudUserID\", \"CloudUserUUID\" or Automatic."];
...
addMessage[General::invallow, "\"Allow\" or \"Disallow\" is expected when specifying constraints."];
addMessage[General::invaprl, "AppearanceRules value `1` must be Branding of Automatic, True, False, or None."];
addMessage[General::invautocp, "AutoCopy value `1` must be either True or False."];
addMessage[General::invbase, "Invalid CloudBase `1`; a fully qualified domain expected."];
addMessage[General::invcloudobj, "`1` is not a valid cloud object."];
addMessage[General::invconstpatt, "Invalid permissions constraint pattern `1`."];
addMessage[General::invconstval, "Invalid value `2` for constraint pattern `1`."];
addMessage[General::invincludedf, "IncludeDefinitions value `1` must be either True or False."]
addMessage[General::invmeta, "Invalid meta information `1`; a list of rules or Association with string keys expected."];
addMessage[General::invperm, "Invalid permissions specification `1`."];
addMessage[General::invpermform, "Permissions `1` is not of the form class -> per"];
addMessage[General::invsharing, "SharingList `1` can only contain strings or PermissionsGroups."];
addMessage[General::invsrc, "Invalid source link `1`."];
...
addMessage[General::invurltp, "CloudObjectURLType `1` must be either \"Object\" or \"Environment\"."];
...
addMessage[General::nonempdir, "Unable to overwrite non-empty directory with `1`."];
...
addMessage[General::unkcountry, "Unknown country `1`"];
addMessage[General::unkdate, "Unable to interprete date `1`"];
...
addMessage[PermissionsGroup::invgrp, "Illegal character in permissions group name `1`."]
...
addMessage[ScheduledTask::rstsch, "Scheduling frequency is restricted on current cloud. Using \"Hourly\" instead."];
...
addMessage[HTTPRequest::invauth,   "Failed to authenticate with `1`. Using `2` instead."];
...
addMessage[CreateCloudUser::notperm, "Admin privilege required to create a user."];
addMessage[CreateCloudUser::notparam, "User already exists."];

addMessage[AddAdminStatus::notperm, "Admin permission is required to create a admin user."];
addMessage[AddAdminStatus::exists, "User already has role `1` assigned."];

addMessage[CloudUserInformation::inv, "User is not a CloudUser."];
addMessage[CloudUserInformation::usernf, "No cloud user found."];

addMessage[CloudUser::inv, "User is not a valid CloudUser."];

addMessage[GetCloudUser::usernf, "No cloud user found."];
