...
With[{CloudObject`Private`dir=DirectoryName[$InputFileName]}, (*change context so this doesn't show in Global`*)
	...
	If[$VersionNumber < 12 && Or[
		(TrueQ[FrontEnd`Private`$KernelInitializationComplete] &&
			MathLink`CallFrontEnd[FrontEnd`Value["$NotebookLicenseType"]] === "Player"),
		$LicenseType === "Player"],
		Get[FileNameJoin[{CloudObject`Private`dir, "Dialogs.Player.m"}]],
    ...
    Get[FileNameJoin[{CloudObject`Private`dir, "UserManagement.m"}]];
]
