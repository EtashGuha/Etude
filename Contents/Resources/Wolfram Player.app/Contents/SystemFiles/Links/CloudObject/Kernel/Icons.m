...
setCloudIcon[Automatic, cloud_, uuid_, iconname_, OptionsPattern[]] :=
    Module[{contentExpr, deployment, iconImage, iconize},
        contentExpr = Check[OptionValue["Content"], Return[$Failed]];
        deployment = Check[OptionValue["Deployment"], Return[$Failed]];
        (* Only use the old name for the iconizing function if it still exists *)
        iconize = Symbol[If[NameQ["Iconize`Iconize"], "Iconize`Iconize", "Iconize`IconizedImage"]];
        iconImage = iconize[Evaluate@contentExpr, deployment,
