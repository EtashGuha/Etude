...
formatToMimeType := formatToMimeType = Composition[
    Replace @ Dispatch[
        Flatten[{
            ...
            "css" -> "text/css",
            "wxf" -> "application/vnd.wolfram.wxf",
            Quiet[
                Map[
                    Replace[
...
CloudImport[failureObj_Failure, args___] := failureObj
...
exportFormToString[expr_, {"HTML", Automatic}, rest___] :=
    exportFormToString[expr, {"HTMLFragment", "text/html;charset=utf-8"}, rest]
...
exportFormToString[expr_, {"WL", mime_}, rest___] := {
    ...
    *)
    Replace[
        $exportFormHandler, {
            ExportString    :> ToString[expr, InputForm, CharacterEncoding -> "ASCII"],
            ExportByteArray :> StringToByteArray[ToString[expr, InputForm, CharacterEncoding -> "ASCII"], "ASCII"],
            (* 
    ...
    formatToMimeMeta[mime, "ascii"]
}

exportFormToString[expr_, {"CloudConnectorExcelForm", Automatic}, rest___] := {
    $exportFormHandler[CloudObject`Private`cloudConnectorExcelForm[expr, rest], "JSON", "Compact" -> True],
...
CloudExport[expr_, format_, CloudObject[uri_, objopts:OptionsPattern[CloudObject]], opts:OptionsPattern[]] :=
	Module[{optsNew, permissions, iconRules, metaInformation},
		Quiet[Catch[
			optsNew = prepareOptionsForWriteObject[CloudExport, {opts, objopts}];
    		...
    		Replace[
            	...
            	{$Failed, ___} :> $Failed,
            	{content_, mimetype_, ___} :> Block[{$CloudBase = handleCBase[OptionValue[CloudExport, {opts}, CloudBase], CloudExport]},
                    Replace[
                        writeObject[CloudObject[uri, FilterRules[{objopts}, Except[CloudObjectNameFormat | CloudObjectURLType]]], content, mimetype["ContentType"], 
                        	permissions, iconRules, Unevaluated[expr], metaInformation, {}, CloudExport],
                        CloudObject[args___] :> CloudObject[args, CloudObjectNameFormat -> Lookup[optsNew, CloudObjectNameFormat], CloudObjectURLType -> Lookup[optsNew, CloudObjectURLType]]
                    ]
                ]}],
        	BadOption], OptionValue::nodef]
		]
...
CloudExport[expr_, format_, failureObj_Failure, opts:OptionsPattern[]] := failureObj
