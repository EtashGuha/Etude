...
CloudShare[failureObj:Failure["CloudObjectInvalidPathFailure", ___], arg_] := failureObj
...
CloudUnshare[failureObj_Failure, arg___] := failureObj
...
updateSharees[cloud_, uuid_, users:{Alternatives[_String, _PermissionsGroup]..}, method_] :=
    Module[{normalizeGroup, usersNew, body},
        normalizeGroup = Map[validatePermissionsClass[#, CloudShare]&, Select[users, MatchQ[#, _PermissionsGroup]& ]];
...
invalidShareeError[data_, normalizedGroup_] :=
    Module[{users = data, failedUsers},
    	If[MatchQ[data, {___, Rule["extraData",__], ___}], users = importFromJSON[Lookup[data, "extraData"]]];
