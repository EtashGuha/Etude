        ...
        */
		...
		for (i = 0; i < len; i++) {
			...
			if (b >= 48 && b <=57) {
				...
				// Accept either . (46) or , (44) as the decimal separator.
				decimalPos = i;
			} else {
		...
		if (digitCount > 0) {
		    String exponent = new String(digitBuf, 0, digitCount);
			try {
                scale -= Integer.parseInt(exponent);
            } catch (NumberFormatException e) {
                // This is intended to catch cases where the number arriving from Wolfram Language has an exponent that is larger then can fit in an int,
                // which is a number that BigDecimal cannot represent. For example, WL $MaxNumber and $MinNumber. Seems sensible to throw
                // one of the standard ArithmeticException values that Java uses.
                throw new ArithmeticException(exponent.startsWith("-") ? "Underflow" : "Overflow");
            }
		}
