//////////////////////////////////////////////////////////////////////////////////////
//
//   J/Link source code (c) 1999-2019, Wolfram Research, Inc. All rights reserved.
//
...
public class JLinkClassLoader extends ClassLoader {
    ...
    /**
     * The mapping that gives the {@link JLinkClassLoaderHelper} which loaded a given package.
     * This is used to try to get that classloader to load all classes for that package,
     * to fix bug 349082.
     * <br />
     * This would have been a {@code Concurrent(Hash)Map}, but as its use is always restricted to a
     * {@code synchronized (lockObject)} block within {@link JLinkClassLoaderHelper}, it should
     * be overkill for this to be an explicitly thread safe object.
     */
    private final Map<Package,JLinkClassLoaderHelper> packageLoaders =
        new HashMap<Package,JLinkClassLoaderHelper>(100);
    ...
    // don't want to override getPackage because it's not *this* classloader that has defined it
    // This is part of the fix for bug 349082.  Renamed from getDefinedPackage because that name is new and final in Java 9.
    Package getPackageDefinedByJLink(String name) { return helper.getPackageDefinedByJLinkHelper(name); }
    ...
    /**
     * The {@link JLinkClassLoaderHelper} which loaded the given package.
     * This is used to try to get that classloader to load all classes for that package,
     * to fix bug 349082.  If the package hasn't been loaded, this will return {@code null}.
     * 
     * @param p the package of interest
     * @return the classloader instance which loaded {@code p}, or {@code null}.
     */
    JLinkClassLoaderHelper getPackageLoader(Package p) {
        return packageLoaders.get(p);
    }
    
    /**
     * Register the given classloader is that for the given package.
     * This method must be invoked for {@link #getPackageLoader(Package)} to return
     * the given classloader for the given package.
     * 
     * @param p the package
     * @param cl the classloader which loaded the package.
     * 
     * @see JLinkClassLoaderHelper#getPackage(String)
     */
    void registerPackage(Package p, JLinkClassLoaderHelper cl) {
       packageLoaders.put(p, cl);
    }
