	...
	private static Expr createAtomicExpr(MathLink ml, int type) throws MathLinkException {
		...
		switch (type) {
			...
			case MathLink.MLTKREAL: {
				...
				// we get the data as a string and interpret it ourselves. It is also the case that the string
				// returned by MathLink will be localized, using comma instead of period as the decimal char. However,
				// the Double(String s) constructor only works with period, regardless of locale, so a simple solution
				// is to blindly convert all commas to periods in the string. There is no way a comma could be in the
				// string unless it represented a decimal point.				
				String s = ml.getString().replace(',', '.');				
				try {
