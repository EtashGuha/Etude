...
public class Install {
	...
	public static void main(String[] args) {
        ...
        File securityConfigFile = null;
        // We want to set up the SecurityManager immediately, so to avoid problems with file permissions
        ...
        for (int i = 0; i < args.length - 1; i++) {
            if (args[i].equalsIgnoreCase("-init")) {
            ...
            } else if (args[i].equalsIgnoreCase("-jlink.securityFile")) {
                ...
                if (securityFileName.contains("..")) {
                    // Don't allow backtracking up out of the securityDir.
                    System.err.println("FATAL ERROR: The -jlink.securityFile command-line argument has an illegal value");
                    return;
                }
                securityConfigFile = new File(securityDir, securityFileName);
        ...
        try {
            ...
            if (securityConfigFile != null) {
                securityManager = new JLinkConfigurableSecurityManager(securityConfigFile);
            } else if (System.getProperty("com.wolfram.jlink.security") != null) {
                // Legacy method. Used by older Cloud installations (pre-10.2).
                String securityManagerClassName = System.getProperty("com.wolfram.jlink.security");
                // Look for security manager class in the same classloader as loaded this class. This means that
                ...
                securityManager = (SecurityManager) securityClass.newInstance();
            } else {
                // Standard desktop security. Only prevents calls to System.exit().
                securityManager = new JLinkSecurityManager();
            }
