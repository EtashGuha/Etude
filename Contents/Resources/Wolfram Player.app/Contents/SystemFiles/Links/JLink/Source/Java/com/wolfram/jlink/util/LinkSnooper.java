...
public class LinkSnooper extends Thread {
    ...
    private String kernelProtocol = "SharedMemory";
    private String kernelLinkHost = null;
    private String kernelLinkOptions = null;
        ...
        while (true) {
            ...
            try {
                if (feMain.ready()) {
                    ...
                    // This is the new method of establishing parallel links, available in v11.1+ of the FrontEnd
                    if (fePreemptive == null && 
                            exprToSafeString(expr.head()).equals("EvaluatePacket") &&
                            exprToSafeString(expr.part(1).head()).equals("FrontEnd`Private`ConnectSecondaryLinks")) {
                        // Now need to read the expr off the link. Throw it away since we already peeked it as expr.
                        feMain.getExpr().dispose();
                        // Snoop looking for EvaluatePacket[FrontEnd`Private`ConnectSecondaryLinks[...]] coming from the FE to the Kernel.
                        ...
                        String serviceName = expr.part(new int[]{1,1}).toString();
                        String preemptiveName = expr.part(new int[]{1,2}).toString();

                        String linkCreateString = "-linkmode listen -linkprotocol " + kernelProtocol;
                        if (kernelLinkHost != null)
                            linkCreateString += " -linkhost " + kernelLinkHost;
                        if (kernelLinkOptions != null)
                            linkCreateString += " -linkoptions " + kernelLinkOptions;
                        else
                            linkCreateString += " -linkoptions MLDontInteract";

                        // These are the links back to the kernel that represent the kernel half of the service and preemptive links.
                        kernelService = MathLinkFactory.createKernelLink(linkCreateString);
                        kernelPreemptive = MathLinkFactory.createKernelLink(linkCreateString);
                        ...
                        // Tell the kernel to connect the links and connect to our side.
                        kernelMain.putFunction("EvaluatePacket", 1);
                        kernelMain.putFunction("FrontEnd`Private`ConnectSecondaryLinks", 3);
                        kernelMain.put(kernelService.name());
                        kernelMain.put(kernelPreemptive.name());
                        kernelMain.put(kernelProtocol);
                        kernelMain.flush();

                        // The definition that the FE sends calls LinkActivate first on the Service link, then then Preemptive link
                        ...
                        feService = MathLinkFactory.createKernelLink("-linkmode connect -linkname " + serviceName);
                        fePreemptive = MathLinkFactory.createKernelLink("-linkmode connect -linkname " + preemptiveName);
                        ...
                        // This fakes the result from the FE back to the kernel for the EvaluatePacket[FrontEnd`Private`ConnectSecondaryLinks[...]].
                        feMain.putFunction("ReturnPacket", 1);
                        feMain.putSymbol("Null");
                        feMain.flush();

                        // Establish the connection with the FE...  We do this after the ReturnPacket[], 
                        // because that kicks the FE into action.
                        feService.connect();
                        ...
                        if (skipInitTraffic && !doPrint) {
                            ...
                            String head = exprToSafeString(expr.head());
                            if (head.equals("EnterTextPacket") || head.equals("EnterExpressionPacket"))
                                doPrint = true;
                        }
                        if (captureMain)
                            output(toKernelArrow + exprToSafeString(expr));
                ...
                if (kernelMain.ready()) {
                    ...
                    if (head.equals("CallPacket")) {
                        String func = exprToSafeString(expr.part(1).head());
                        if (func.equals("FrontEnd`SetKernelSymbolContexts") ||
                            ...
                            exprToPrint = "CallPacket[" + exprToSafeString(expr.part(1).head()) + "[ -- large contents not printed -- ]]";
                        } else {
                            exprToPrint = exprToSafeString(expr);
                        }
                    } else {
                        exprToPrint = exprToSafeString(expr);
                    }
                    ...
                    // This is the old method of establishing parallel links, used in v6.0 - v11.0 of the FrontEnd
                    if (fePreemptive == null &&
                            head.equals("CallPacket") &&
                            exprToSafeString(expr.part(1).head()).equals("FrontEnd`OpenParallelLinksPacket")) {
                        // Now need to read the expr off the link. Throw it away since we already peeked it as expr.
                ...
                if (fePreemptive != null && fePreemptive.ready()) {
                    ...
                    if (capturePre) {
                        ...
                        output(preemptiveLinkPrefix + toKernelArrow + exprToSafeString(expr));
                        expr.dispose();
                    }
                ...
                if (kernelPreemptive != null && kernelPreemptive.ready()) {
                    ...
                    if (capturePre) {
                        ...
                        output(preemptiveLinkPrefix + toFEArrow + exprToSafeString(expr));
                        expr.dispose();
                    }
                ...
                if (feService != null && feService.ready()) {
                    ...
                    if (captureServ) {
                        ...
                        output(serviceLinkPrefix + toKernelArrow + exprToSafeString(expr));
                        expr.dispose();
                    }
                ...
                if (kernelService != null && kernelService.ready()) {
                    ...
                    if (captureServ) {
                        ...
                        output(serviceLinkPrefix + toFEArrow + exprToSafeString(expr));
                        expr.dispose();
                    }
    ...
    // Utility method to catch any exceptions thrown by Expr.toString(). This was added to avoid
    // NumberFormatException when trying to create a BigDecimal out of a Mathematica big real. M can
    // have real numbers that won't fit into a BigDecimal (because the exponent is outside the range
    // a Java int).
