...
OAuthSigning`OAuthAuthentication[rules__?OptionQ] :=
    Block[{token, auth,$BlockingDialog=Lookup[rules,"Blocking",!TrueQ[$CloudEvaluation]]},
        ...
        (
            ...
            If[token === $Canceled,
                $Canceled,
                OAuthSigning`OAuthToken[auth, token]
            ] /; (token =!= $Failed)
        ) /; (auth =!= $Failed)
...
OAuthSigning`OAuthSignURL[url_, opts__?OptionQ] :=
    Block[{res},

        res = OAuthURLFetchList[
                url,
                Flatten[{opts}] /. OAuthSigning`OAuthToken -> List
            ];
        res /; ListQ[res]
...
OAuthSigning`OAuthAuthentication::libload = "Failed to load OAuth libraries.";


(* OAuth configuration *)

OAuthSigning`OAuthAuthentication::type = "`2` in `1` must be one of `3`.";
OAuthSigning`OAuthAuthentication::stype = "`2` in `1` is not a string.";
OAuthSigning`OAuthAuthentication::ntype = "Failed to automatically determine authorization type, please specify the `1` property.";
OAuthSigning`OAuthAuthentication::prop = "The rules `2` give values for things that are not valid `1` properties.";
OAuthSigning`OAuthAuthentication::sane = "`1` sanity checking failed for properties `2`.";


(* Get request token *)

OAuthSigning`OAuthAuthentication::illreq = "The service `1` is of type `2` and does not use request tokens.";
OAuthSigning`OAuthAuthentication::reqbdy = "The service `1` returned invalid request token response `2`.";


(* Assemble authorization URL *)

OAuthSigning`OAuthAuthentication::sclst = "The scope `3` must be a list of strings for authorization type `1` used by `2`.";
OAuthSigning`OAuthAuthentication::scnorlst = "The scope `3` must be None or a list of strings for authorization type `1` used by `2`.";
OAuthSigning`OAuthAuthentication::scpar = "OAuth configuration for `1` needs a value for `2` in order to authorize with explicit scope.";
OAuthSigning`OAuthAuthentication::stfmt = "The `1` state `3` for `2` must be a non-empty string of word characters or the symbol None.";


(* Get access token *)

OAuthSigning`OAuthAuthentication::bdrt = "`3` is not a valid request token for authorization type `1` usd by `2`.";
OAuthSigning`OAuthAuthentication::rtnone = "Request token None is not valid for authorization type `1` used by `2`.";
OAuthSigning`OAuthAuthentication::rtenone = "Request token `3` should be None for authorization type `1` used by `2`.";
OAuthSigning`OAuthAuthentication::accbdy = "The service `1` returned invalid access token response `2`.";
OAuthSigning`OAuthAuthentication::bdstate = "The `1` access token from `2` was discarded because of a possible cross site request forgery attack. Make sure that the `3` belongs to the autorization request initiated by Mathematica.";


(* OAuth flow *)

OAuthSigning`OAuthAuthentication::reqfail = "Failed to obtain OAuth request token for `1`.";
OAuthSigning`OAuthAuthentication::bdver = "The authorization verifier `1` must be a string.";
OAuthSigning`OAuthAuthentication::accfail = "Failed to obtain OAuth access token for `1`.";
OAuthSigning`OAuthAuthentication::bdkey = "The authorization key `1` must be a string.";


(* Sign requests *)

OAuthSigning`OAuthSignURL::nopt = "The option `2` is missing in `1`.";
OAuthSigning`OAuthSignURL::stropt = "URL parameter options `2` for `1` must have string names and string values.";
...
messageOAuthAuthentication[Automatic, ___] :=
    Message[OAuthSigning`OAuthAuthentication::ntype, "OAuthVersion"]

messageOAuthAuthentication[authtype:Except[_String], ___] :=
    Message[OAuthSigning`OAuthAuthentication::stype, "OAuthVersion" -> authtype, authtype]

messageOAuthAuthentication[authtype_, ___] :=
    Message[OAuthSigning`OAuthAuthentication::type, "OAuthVersion" -> authtype, authtype, OAuthService["OAuthVersion"]]
...
OAuthFlow[auth_?OAuthSigning`Private`AuthQ, args___] :=
    Block[{token},
...
OAuthSigning`OAuthToken /: MakeBoxes[e:OAuthSigning`OAuthToken[auth_, tok_], format_] :=
...
(* ========================================================================= ==
    ...
    Comments:
        OAuthSigning`Private`AuthQ gives True for any expression holding an authorization
        configuration.  Each type of authorization must overload OAuthSigning`Private`AuthQ to
...
(* OAuthSigning`Private`AuthQ *)

OAuthSigning`Private`AuthQ[_] := False
...
TokenSaneQ[_, _OAuthSigning`Private`temporaryOAuthToken] :=
    True/;!$BlockingDialog
...
messageOAuthURLFetchList[args___, opts:OptionsPattern[]] /; (FilterRules[{opts}, "OAuthAuthentication"] === {}) :=
    Message[OAuthSigning`OAuthSignURL::nopt, HoldForm[OAuthURLFetchList[args, opts]], "OAuthAuthentication"]
...
oAuthURLFetchList[args___, opts:OptionsPattern[]] :=
    Block[{authrules, authopt, auth, token, otheropts, res},
        ...
        (
            ...
            ) /; OAuthSigning`Private`AuthQ[auth]
        ) /; authrules =!= {}
    ]
...
getAuthFromOpts[auth_?OAuthSigning`Private`AuthQ] := auth

getAuthFromOpts[opts_List] :=
    Block[{res},
        res = Cases[opts, _?OAuthSigning`Private`AuthQ];
...
nonblockingVerifierDialog[auth_, authorizeURL_, redirectFun_] :=
    Block[{customDialog, serviceName, verifierLabel, inputFieldOpts, title, prompt1, prompt2, temptoken=Unique[]},
        ...
        OAuthSigning`Private`temporaryOAuthToken[temptoken]
]
...
flowGetRequestToken[auth_] :=
    Block[{res, issued = False},
        ...
        If[res === $Failed && !issued,
            Message[OAuthSigning`OAuthAuthentication::reqfail, takeServiceName[auth]]
        ];
        res
    ]
...
messageFlowExtractCode[data:Except[_String], extractor_] :=
    Message[OAuthSigning`OAuthAuthentication::bdver, data]
...
flowGetAccessToken[auth_, requestToken_, code_, state___] :=
    Block[{res, issued = False},
        ...
        If[res === $Failed && !issued,
            Message[OAuthSigning`OAuthAuthentication::accfail, takeServiceName[auth]]
        ];
        res
    ]
...
messageGetParameters[opts:{___?OptionQ}, auth_] :=
    Block[{params},
        params = Replace["Parameters" /. opts, "Parameters" -> {}, {0}];
        (
            Message[OAuthSigning`OAuthSignURL::stropt, takeServiceName[auth], Cases[params, Except[_String -> _String]]]
...
OAuthSigning`Private`AuthQ[_OAuth10Parameters] := True
...
iOAuthAuthentication[$authKey10, opts:{___?OptionQ}] :=
    Block[{defaults, badopts, res, tests, sane},
        ...
        If[badopts =!= {},
            Message[OAuthSigning`OAuthAuthentication::prop, AuthType[$authKey10], badopts]
        ];
        (
            ...
            If[!sane && ListQ[tests],
                Message[OAuthSigning`OAuthAuthentication::sane, AuthType[$authKey10], Cases[tests, _[t_, Except[True]] :> t]]
            ];
            res /; sane
        ) /; (badopts === {})
    ]
...
getRequestToken10[auth_OAuth10Parameters] :=
    Block[{requestEndpoint, requestVerb, consumerKey, consumerSecret, redirectURI,
        ...
        (
            ...
            tokenBody = URLFetch[signedURL, "Method" -> requestVerb, verifyPeer, "CredentialsProvider" -> None];
            (
                tokens = extractToken10[tokenBody, tokenExtractor, auth, Hold[OAuthSigning`OAuthAuthentication::reqbdy]];
                tokens /; tokenSaneQ10[tokens]
            ) /; StringQ[tokenBody]
...
messageAssembleAuthorizationURL10[auth_OAuth10Parameters, tok:Except[_Token10], _] :=
    Message[OAuthSigning`OAuthAuthentication::bdrt, AuthType[auth], takeServiceName[auth], tok]

messageAssembleAuthorizationURL10[auth_OAuth10Parameters, _, sc:Except[None | {___String}]] :=
    Message[OAuthSigning`OAuthAuthentication::scnorlst, AuthType[auth], takeServiceName[auth], sc]

messageAssembleAuthorizationURL10[auth_OAuth10Parameters, _, scope:{___String}] /; (takeScopeParameter[auth] === None) :=
    Message[OAuthSigning`OAuthAuthentication::scpar, takeServiceName[auth], ScopeParameter]
...
getAccessToken10[auth_OAuth10Parameters, requestToken_Token10, verifier_] :=
    Block[{consumerKey, consumerSecret, accessEndpoint, accessVerb,
        ...
        (
            ...
            tokenBody = URLFetch[signedURL, "Method" -> accessVerb, verifyPeer, "CredentialsProvider" -> None];
            (
                accessToken = extractToken10[tokenBody, tokenExtractor, auth, Hold[OAuthSigning`OAuthAuthentication::accbdy]];
...
messageGetAccessToken10[auth_OAuth10Parameters, None, _] :=
    Message[OAuthSigning`OAuthAuthentication::rtnone, AuthType[auth], takeServiceName[auth]]
...
messageGetAccessToken10[auth_OAuth10Parameters, _, ver:Except[_String]] :=
    Message[OAuthSigning`OAuthAuthentication::bdver, ver]
...
tokenSaneQ20[tok_] := MatchQ[tok, Token20[_String | {(_String|_Integer) ..}]]

OAuthSigning`Private`AuthQ[_OAuth20Parameters] := True
...
(* ========================================================================= ==
    ...
    Comments:
		...
		"AuthorizationFunction" is the function for requesting the authorization code
		"AccessTokenRequestor" is the function for requesting the access token
    ...
    Block[{commontests, moretests},
        ...
        moretests = {
            ...
            "RedirectURI" ->
                nonEmptyStringQ[takeRedirectURI[auth]],
            "AccessTokenExtractor" ->
                Or[MemberQ[OAuthService["TokenExtractor"], takeAccessTokenExtractor[auth]],
                MatchQ[takeAccessTokenExtractor[auth],_Function]],
            "AutomaticScope" ->
            ...
            "AuthorizationFunction" ->
                With[{authfun = takeAuthorizationFunction[auth]},
                    authfun === Automatic || MatchQ[authfun, _Function|_String]
                ],
            "AccessTokenRequestor" ->
                With[{requestor = takeAccessTokenRequestor[auth]},
                    requestor === Automatic ||requestor === "HTTPBasic" ||
                    MatchQ[requestor, _Function|_String]
                ]
        };
        Join[commontests, moretests]
    ]
...
takeAuthorizationFunction[auth_OAuth20Parameters]	:= auth[[16]]
takeAccessTokenRequestor[auth_OAuth20Parameters]	:= auth[[17]]
...
iOAuthAuthentication[$authKey20, opts:{___?OptionQ}] :=
    Block[{defaults, badopts, res, tests, sane},
        ...
        badopts = FilterRules[opts, Except[defaults]];
        If[badopts =!= {},
            Message[OAuthSigning`OAuthAuthentication::prop, AuthType[$authKey20], badopts]
        ];
        (
            ...
            sane = TrueQ[And @@ (Last /@ tests)];
            If[!sane && ListQ[tests],
                Message[OAuthSigning`OAuthAuthentication::sane, AuthType[$authKey20], Cases[tests, _[t_, Except[True]] :> t]]
            ];
            res /; sane
        ) /; (badopts === {})
    ]
...
messageGetRequestToken20[auth_OAuth20Parameters] :=
    Message[OAuthSigning`OAuthAuthentication::illreq, takeServiceName[auth], AuthType[auth]]
...
AssembleAuthorizationURL[auth_OAuth20Parameters, requestToken_, scope_, state_:None] :=
    Block[{res, authfun= takeAuthorizationFunction[auth]},
        res = assembleAuthorizationURL20[authfun, auth, requestToken, scope, state];
        res /; (StringQ[res] || res === $Failed)
    ]
...
assembleAuthorizationURL20[Automatic, auth_OAuth20Parameters, requestToken:None, scope:{___String}, state:None] :=
...
assembleAuthorizationURL20[Automatic, auth_OAuth20Parameters, requestToken:None, scope:{___String}, state_String /; StringMatchQ[state, RegularExpression["\\w+"]]] :=
    Block[{authorizeEndpoint, consumerKey, redirectURI, res},
        authorizeEndpoint = takeAuthorizeEndpoint[auth];
        consumerKey = takeConsumerKey[auth];
        redirectURI = takeRedirectURI[auth];
...
assembleAuthorizationURL20[fun_Function, auth_OAuth20Parameters, requestToken:None, scope:{___String}, state:None] :=
    Block[{authorizeEndpoint, consumerKey, redirectURI, res},
        authorizeEndpoint = takeAuthorizeEndpoint[auth];
        consumerKey = takeConsumerKey[auth];
        redirectURI = takeRedirectURI[auth];
...
assembleAuthorizationURL20[fun_Function, auth_OAuth20Parameters, requestToken:None, scope:{___String}, state_String /; StringMatchQ[state, RegularExpression["\\w+"]]] :=
    Block[{authorizeEndpoint, consumerKey, redirectURI, res},
        authorizeEndpoint = takeAuthorizeEndpoint[auth];
        consumerKey = takeConsumerKey[auth];
        redirectURI = takeRedirectURI[auth];
...
messageAssembleAuthorizationURL20[_, auth_OAuth20Parameters, tok:Except[None], _, _] :=
    Message[AssembleAuthorizationURL::rtenone, AuthType[auth], takeServiceName[auth], tok]

messageAssembleAuthorizationURL20[_, auth_OAuth20Parameters, _, sc:Except[{___String}], _] :=
    Message[OAuthSigning`OAuthAuthentication::sclst, AuthType[auth], takeServiceName[auth], sc]

messageAssembleAuthorizationURL20[_, auth_OAuth20Parameters, _, _, state:Except[_String | None]] :=
    Message[OAuthSigning`OAuthAuthentication::stfmt, AuthType[auth], takeServiceName[auth], state]

messageAssembleAuthorizationURL20[_, auth_OAuth20Parameters, _, _, state_String /; !StringMatchQ[state, RegularExpression["\\w+"]]] :=
    Message[OAuthSigning`OAuthAuthentication::stfmt, AuthType[auth], takeServiceName[auth], state]
...
getAccessToken20[auth_OAuth20Parameters, requestToken:None, verifier_, state_] :=
    Block[{ tokenExtractor, tokenBody, tmp, accessToken, state2,requestor},
        tokenExtractor = OAuthService["TokenExtractor", takeAccessTokenExtractor[auth]];
        requestor=takeAccessTokenRequestor[auth];
        (

            tokenBody=getaccessToken20[requestor, auth, verifier];
            (
                tmp = extractToken20[tokenBody, tokenExtractor, auth, Hold[OAuthSigning`OAuthAuthentication::accbdy]];
                (
...
getaccessToken20["HTTPBasic",auth_OAuth20Parameters, verifier_] :=
    Block[{consumerKey, consumerSecret, accessEndpoint, accessVerb, redirectURI,
            verifyPeer, accessToken, state2},
        consumerKey = takeConsumerKey[auth];
        consumerSecret = takeConsumerSecret[auth];
...
getaccessToken20[fun_Function,auth_OAuth20Parameters, verifier_] :=
    Block[{consumerKey, consumerSecret, accessEndpoint, accessVerb, redirectURI,
            verifyPeer, accessToken, state2},
        consumerKey = takeConsumerKey[auth];
        consumerSecret = takeConsumerSecret[auth];
...
messageGetAccessToken20[auth_OAuth20Parameters, tok:Except[None], _, _] :=
    Message[OAuthSigning`OAuthAuthentication::rtenone, AuthType[auth], takeServiceName[auth], tok]

messageGetAccessToken20[auth_OAuth20Parameters, _, ver:Except[_String], _] :=
    Message[OAuthSigning`OAuthAuthentication::bdver, ver]

messagegetaccessToken20[_, auth_OAuth20Parameters, ver:Except[_String]] :=
    Message[OAuthSigning`OAuthAuthentication::bdver, ver]
...
messageIGetAccessToken20[auth_OAuth20Parameters, tok_, state_String, state2_String] /;
        state =!= state2 :=
    Message[OAuthSigning`OAuthAuthentication::bdstate, AuthType[auth], takeServiceName[auth], takeVerifierLabel[auth]]
...
oAuthFlow20[auth_OAuth20Parameters, scope_] :=
    Module[{scope2, requestToken, authorizeURL, state, res},
        ...
        res= With[{st=state,gatDefinition=DownValues[OAuthSigning`Private`getAccessToken20]},
                nonblockingVerifierDialog[auth, authorizeURL,
                    (Block[{OAuthSigning`Private`getAccessToken20,$BlockingDialog=False},
                        DownValues[OAuthSigning`Private`getAccessToken20]=gatDefinition;
...
OAuthSigning`Private`AuthQ[_OAuthSLParameters] := True
...
iOAuthAuthentication[$authKeySL, opts:{___?OptionQ}] :=
    Block[{defaults, badopts, res, tests, sane},
        ...
        badopts = FilterRules[opts, Except[defaults]];
        If[badopts =!= {},
            Message[OAuthSigning`OAuthAuthentication::prop, AuthType[$authKeySL], badopts]
        ];
        (
            ...
            sane = TrueQ[And @@ (Last /@ tests)];
            If[!sane && ListQ[tests],
                Message[OAuthSigning`OAuthAuthentication::sane, AuthType[$authKeySL], Cases[tests, _[t_, Except[True]] :> t]]
            ];
            res /; sane
        ) /; (badopts === {})
    ]
...
getAccessTokenSL[auth_OAuthSLParameters, requestToken:None, wolframKey_String, state_String] :=
    Block[{accessEndpoint, keyStr, secretStr, tokenBody, tmp, accessToken, state2, serverEmulator},
        ...
        (
            tmp = extractTokenSL[tokenBody, wolframTokenExtractor, auth, Hold[OAuthSigning`OAuthAuthentication::accbdy]];
            (
                {accessToken, state2} = tmp;
...
messageGetAccessTokenSL[auth_OAuthSLParameters, _, key:Except[_String]] :=
    Message[OAuthSigning`OAuthAuthentication::bdkey, key]
...
messageIGetAccessTokenSL[auth_OAuthSLParameters, tok_, state_String, state2_String] /;
        state =!= state2 :=
    Message[OAuthSigning`OAuthAuthentication::bdstate, "OAuth", takeServiceName[auth], "key"]
...
OAuthService["TokenExtractor", "Refresh/2.0"] := jsonAccessTokenAndRefreshExtractor
...
(* jsonAccessTokenAndRefreshExtractor *)

jsonAccessTokenAndRefreshExtractor[body_String] :=
     Block[ {rules, tokens, state},
         rules = Quiet@ImportString[body, "JSON"];
         (
	         tokens = Lookup[rules, {"access_token", "refresh_token", "expires_in"}];
	         tokens = If[ MatchQ[tokens, {(_String|_Integer)..}] && Length[tokens] === 3,
	                      tokens,
	                      $Failed
	                  ];
			state = FilterRules[rules, "state"];
	        state = If[MatchQ[state, {_ -> _String}], state[[1, 2]], None];
			{
				"Token" -> tokens,
				"State" -> state
			} /; (MatchQ[tokens,{( _String|_Integer)..}] && MatchQ[state, _String | None])
        ) /; MatchQ[rules, {___Rule}]
     ];


jsonAccessTokenAndRefreshExtractor[_] := $Failed
...
messageHMACSha1SignatureService[expr___] /; !initializedQ :=
    Message[OAuthSigning`OAuthAuthentication::libload]
