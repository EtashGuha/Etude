...
BeginPackage["CCodeGenerator`", 
	{"CompiledFunctionTools`", "CompiledFunctionTools`Opcodes`","SymbolicC`", "CCompilerDriver`"}]
(* Exported symbols added here with SymbolName::usage *) 

LibraryGenerate::usage = "LibraryGenerate[ cfun, name, opts] generates a shared library from the compiled function cfun using name as the exported function name."

CCodeGenerate::usage = "CCodeGenerate[ cfun, name, opts] generates C code from the compiled function cfun using name as the exported function name and saves in the file name.c.  CCodeGenerate[ cfun, name, filename] generates output in filename. CCodeGenerate[ cfun, name, {filename, headerfile}] generates any C header output in headerfile."

SymbolicCGenerate::usage = "SymbolicCGenerate[ cfun, name, opts] generates symbolic C from the compiled function cfun using name as the exported function name."

CCodeStringGenerate::usage = "CCodeStringGenerate[ cfun, name, opts] generates C code from the compiled function cfun using name as the exported function name."

$CCodeGenerateDirectory::usage ="$CCodeGenerateDirectory is the location of the C Code Generate Package."

CCodeExport::usage = "CCodeExport[ cfun, filename] exports the compiled function cfun as C code using the file filename. CCodeExport[ cfun, {filename, headerfilename}] also exports a header file into the file headerfilename."

CCodeGenerate::wmreq = "The expression `1` requires Mathematica to be evaluated.   The function will be generated but can be expected to fail with a nonzero error code when executed."

CCodeGenerate::nosupp = "The expression `1` is not supported for code generation."

CCodeGenerate::nodp = "The tensor register `1` has not been correctly set up for code generation operations."


Begin["`Private`"]

CCodeExport[ cfun_CompiledFunction, fileNameIn_] :=
	Module[ {cproc, files, fileName, name, headerName},
		...
		CCodeGenerate[ cproc, name, fileName, "CodeTarget" -> "WolframRTL", "HeaderName" -> headerName];
		If[ Length[ files] > 1
				,
				CCodeGenerate[ cproc, name, Part[files,2], "CodeTarget" -> "WolframRTLHeader"];
		];
		fileNameIn
	]


$CCodeGenerateDirectory = DirectoryName[ System`Private`$InputFileName]

Options[CCodeStringGenerate] = {"CodeTarget" -> "WolframRTL", "WrapperFunction" -> None, "HeaderName" -> Automatic, "LifeCycleFunctionNames" -> Automatic}


CCodeStringGenerate[cfun: Except[_List], name:Except[_List], opts:OptionsPattern[]] :=
	CCodeStringGenerate[{cfun}, {name}, opts]
	
CCodeStringGenerate[{cfun_CompiledFunction, funs___}, {name_String, names___}, opts:OptionsPattern[]] :=
	CCodeStringGenerate[{ToCompiledProcedure[cfun], funs}, {name, names}, opts]
	
CCodeStringGenerate[{cproc_CompiledProcedure, cprocs___}, {name_String, names___}, opts:OptionsPattern[]] := 
Module[{symbC},
...
Options[SymbolicCGenerate] = {"CodeTarget" -> "WolframRTL", "WrapperFunction" -> None, "HeaderName" -> Automatic, "LifeCycleFunctionNames" -> Automatic}

SymbolicCGenerate[cfun: Except[_List], name:Except[_List], opts:OptionsPattern[]] :=
	SymbolicCGenerate[{cfun}, {name}, opts]

SymbolicCGenerate[{cfun_CompiledFunction, funs___}, {name_String, names___}, opts:OptionsPattern[]] :=
	SymbolicCGenerate[{ToCompiledProcedure[cfun],funs}, {name, names}, opts]

SymbolicCGenerate[{cproc_CompiledProcedure,funs___}, {name_String, names___}, opts:OptionsPattern[]] := 
	SymbolicCGenerateMain[cproc, name, {funs}, {names}, FilterRules[{opts}, Options[SymbolicCGenerateMain]]]


Options[CCodeGenerate] = {"CodeTarget" -> "WolframRTL", "WrapperFunction" -> None, "HeaderName" -> Automatic, "LifeCycleFunctionNames" -> Automatic}


CCodeGenerate[cfun: Except[_List], name:Except[_List], opts:OptionsPattern[]] :=
	CCodeGenerate[{cfun}, {name}, opts]

CCodeGenerate[cfun: Except[_List], name:Except[_List], filename:Alternatives[_String, _File], opts:OptionsPattern[]] :=
	CCodeGenerate[{cfun}, {name}, filename, opts]	

CCodeGenerate[{cfun_CompiledFunction, funs___}, {name_String, names___}, opts:OptionsPattern[]] :=
	CCodeGenerate[{ToCompiledProcedure[cfun], funs}, {name, names}, Automatic, opts]
	
CCodeGenerate[ {cfun_CompiledFunction, funs___}, {name_String, names___}, filename_String, opts:OptionsPattern[]] :=
	CCodeGenerate[{ToCompiledProcedure[cfun], funs}, {name, names}, filename, opts]
	
CCodeGenerate[ {cfun_CompiledFunction, funs___}, {name_String, names___}, File[filename_String], opts:OptionsPattern[]] :=
	CCodeGenerate[ {cfun, funs}, {name, names}, filename, opts]	
	
CCodeGenerate[ {cfun_CompiledProcedure, funs___}, {name_String, names___}, opts:OptionsPattern[]] :=
	CCodeGenerate[{cfun, funs}, {name, names}, Automatic, opts]
	
CCodeGenerate[{cproc_CompiledProcedure, funs___}, {name_String, names___}, filenameIn:(_String|Automatic), opts:OptionsPattern[]] := 
Module[{symbC, codestring, codeTarget, filename},
	...
	If[symbC === $Failed, Return[$Failed, Module]];
	codestring = ToCCodeString[symbC];
...
LibraryGenerate[cfun: Except[_List], name:Except[_List], opts:OptionsPattern[]] :=
	LibraryGenerate[{cfun}, {name}, opts]
		
LibraryGenerate[{cfun_CompiledFunction, funs___}, {name_String, names___}, opts:OptionsPattern[]] :=
	LibraryGenerate[{ToCompiledProcedure[cfun], funs}, {name, names}, opts]
		
Options[LibraryGenerate] = {"CodeTarget" -> "WolframLibrary", "WrapperFunction" -> None, "TargetDirectory" -> Automatic, "LifeCycleFunctionNames" -> Automatic}
		
LibraryGenerate[{cproc_CompiledProcedure,funs___}, {name_String, names___}, opts:OptionsPattern[]] :=
Module[{symbC, cstring, codeTarget},
	codeTarget = OptionValue[ LibraryGenerate, FilterRules[{opts}, Options[LibraryGenerate]], "CodeTarget"];
...
GenerateRTL[ cproc_, cstring_, name_, opts:OptionsPattern[]] :=
	Module[ {dir, libDir, hname},
		...
		dir = OptionValue[ LibraryGenerate, FilterRules[{opts}, Options[LibraryGenerate]], "TargetDirectory"];
		If [!StringQ[ dir], Return[ $Failed, Module]];
		hname = FileNameJoin[ {dir, name <> ".h"}];
		CCodeGenerate[ cproc, name, hname, "CodeTarget" -> "WolframRTLHeader", FilterRules[{opts}, Options[CCodeGenerate]]];
...
Options[SymbolicCGenerateMain] = {"CodeTarget"->"WolframRTL", "WrapperFunction"->None, "CallerData"->Null, "HeaderName" -> Automatic, "LifeCycleFunctionNames" -> Automatic};
...
initObjData[ objData_, lines_, codeTarget_, callerData_, wrapper_, cons_, name_, proc_] :=
Module[{cinfo = First[proc]},
	objData[MTensor] = False;
	...
	objData[ResultMArgument] = (callerData === Null) && !objData[WolframRTL];
...
convertFullType[ MTensor[ type_, rank_]] := {convertType[type], rank}
...
dllFunctionArgumentType[MTensor[ type_, rank_]] := {convertType[type], rank, "Constant"}
...
typeToCType[ MTensor] = "MTensor"
typeToCType[ MTensor[ _,_]] = "MTensor"
...
getTensorEnumType[ Register[ MTensor[ type_, rank_], num_]] := ToTypeEnum[type]

getRank[ Register[ MTensor[ type_, rank_], num_]] = rank
getRank[Register[type_, num_]] = 0;

getType[Register[MTensor[type_, rank_], num_]] = type
getType[Register[type_, num_]] = type;

isTensor[ Register[ MTensor[ type_, rank_], num_]] := True

isTensor[ Register[ type_, num_]] := False 

RankZeroTensorQ[Register[MTensor[type_, 0], _]] := True;
...
getCType[ Register[ MTensor[type_,rank_], reg_]] :=
	typeToCType[MTensor]
	
MArgumentName[Register[_MTensor, _]] := "MTensor"
MArgumentName[Register[type_Symbol, _]] := SymbolName[type]

MArgumentGetFunction[reg_] := StringJoin["MArgument_get", MArgumentName[reg]]
MArgumentGetAddressFunction[reg_] := StringJoin["MArgument_get", MArgumentName[reg], "Address"]
MArgumentSetFunction[reg_] := StringJoin["MArgument_set", MArgumentName[reg]]
...
makeAtom[ Register[MTensor[type_, _], num_], objData_] :=
	makeAtom[ typeToVariable[MTensor], num, objData]
...
(*
	Like makeAtom, but dereferences MTensor variables
*)
makeTDAtom[Register[MTensor[type_, _], num_], objData_] :=
	CDereference[makeAtom[ typeToVariable[MTensor], num, objData]]
...
*)
makeAtomAddress[reg:Register[MTensor[type_, _], num_], objData_] := makeAtom[reg, objData]
...
(* Data pointers used to inline vector setting/getting.
   ...
   Whenever a MTensor register, reg,  is modified in a way that
...
getMTensorDataFunction[objData_, type_] := "MTensor_get" <> ToString[type] <> "DataMacro"
	...
	objData[DataPointer[reg]] := (
		If[UndefinedQ[objData[DataPointerCode[var]]],
			objData[DataPointerDeclaration[count, locality]] = CDeclare[DPDeclareType[type, locality], CDereference[var]];
     		objData[DataPointerCode[var]] = CAssign[var, CCall[getMTensorDataFunction[objData, type], {makeTDAtom[reg, objData]}]]
     	];
   		var
    )
)

getMTensorDimensionsFunction[objData_] := "MTensor_getDimensionsMacro"
 	...
 	objData[DataDimensions[reg]] := (
		If[UndefinedQ[objData[DataDimensionsCode[var]]],
			objData[DataDimensionsDeclaration[count, locality]] = CDeclare[DPDeclareType[Integer, locality], CDereference[var]];
     		objData[DataDimensionsCode[var]] = CAssign[var, CCall[getMTensorDimensionsFunction[objData], {makeTDAtom[reg, objData]}]]
     	];
  	 	var
 	)
)

getMTensorNElemsFunction[objData_] := "MTensor_getFlattenedLengthMacro"
 	...
 	objData[DataNElems[reg]] := (
		If[UndefinedQ[objData[DataNElemsCode[var]]],
			objData[DataNElemsDeclaration[count, locality]] = CDeclare[DPDeclareType[Integer, locality], var];
     		objData[DataNElemsCode[var]] = CAssign[var, CCall[getMTensorNElemsFunction[objData], {makeTDAtom[reg, objData]}]]
     	];
  	 	var
    );
)
...
setDataPointer[objData_, reg:Register[MTensor[type_, rank_], num_]] := 
...
DataPointerCheck[None, reg_, objData_] := Throw[Message[CCodeGenerate::nodp, reg]; $Failed, "UnsetDataPointer"]
...
unsetDataPointer[reg:Register[MTensor[rank_, type_], num_]] := If[StringQ[DataPointer[reg]], Unset[DataPointer[reg]]]
...
getDerefPointer[ Register[ MTensor[ type_, _], num_]] :=
	ToString[typeToCType[ MTensor]] <> "*"
...
getArrayReference[ objData_, reg:Register[MTensor[__], _], arrRef_] :=
	CCall[MArgumentGetAddressFunction[reg], {arrRef}]

getArrayReference[ objData_, reg_, arrRef_] :=
	CCall[MArgumentGetFunction[reg], {arrRef}]
...
makeArgumentArrayAssign[ objData_, Instruction[Set, reg_, arg_], argNum_] :=
	Module[ {arrRef},
		...
		{
			CAssign[ makeAtom[ reg, objData], getArrayReference[ objData, reg, arrRef]],
	...
	Module[ {resRef, resReg, resVar, resT},
		...
		resVar = objData[ resVariable];
		{
			...
			,
				...
				If[isTensor[resReg],
					If[objData[ArrayArgs] && objData[ ResultMArgument],
						resT = CCall[MArgumentGetAddressFunction[resReg], {resRef}];
					(* else *), 
						resT = resRef;
					];
					CCall[objData[structCompile["MTensor_copy"]], {resT, CDereference[resVar]}]
			(* else *),
					If[ objData[ ResultMArgument],
						CCall[MArgumentSetFunction[resReg], {resRef, makeAtom[resReg, objData]}]
						,
			...
			If[StringQ[objData[TensorLocals]],
				CCall[
					objData[structCompile["ReleaseInitializedMTensors"]], 
...
buildLocal[ objData_, {type_, num_}] :=
	Module[ {cType, baseName, locals, decls, count},
		...
		decls = declareLocals[objData, cType, locals, type === MTensor, count];
		If[(type === MTensor) && (Length[locals] > 0),
			objData[MTensor] = True;
			...
			If[Length[locals] > count, 
				...
				objData[TensorLocals] = "Tinit";
				decls = Append[decls, CDeclare["MTensorInitializationData", objData[TensorLocals]]];
				objData[TensorLocalsInit] = {
					CAssign[objData[TensorLocals], 
						CCall[objData[structCompile["GetInitializedMTensors"]], {objData[LibraryData], Length[locals]}]],
					Table[
						CAssign[locals[[i]], 
							CCall["MTensorInitializationData_getTensor", {objData[TensorLocals], i - 1}]],
						{i, Length[locals]}
					]
				};
			];
...
Module[{tc = makeTDAtom[r, objData]},
	CIf[tc, 
		CBlock[
			CCall[objData[struct["MTensor_free"]], {tc}],
			CAssign[tc, 0]
		]
	]
];
...
Options[ SymbolicCGenerateDriver] = {"CodeTarget" -> "WolframRTL", "CallerData" -> Null, "WrapperFunction"->None, "HeaderName" -> Automatic, "LifeCycleFunctionNames" -> Automatic}
...
Catch[
	...
	Module[ {resType, funArgs, localVars, initCode, assignArgs, body, code, 
			cleanCode, retCode, function,  codeTarget, callerData, wrapper, consCode, extraCode,
		...
		initObjData[ objData, lines, codeTarget, callerData, wrapper, cons, name, proc];
		...
		If[objData[DLLFunctionArgNum] > 0,
			localVars = {localVars, CDeclare["MArgument", 
		...
		Switch[codeTarget, 
			...
			"WolframRTLHeader",
					{
					buildThisHeaderFile[objData, name,  CFunction[ {"EXTERN_C", "DLLEXPORT", resType}, name, funArgs]],
					extraCode
					},
			"NestedFunction",
			...
			"ExtraWolframRTLHeader",
				CFunction[ {"EXTERN_C", "DLLEXPORT", resType}, name, funArgs],
			_,
				CProgram[
...
getExtraCode[ objData_, codeTarget_, {cfun_CompiledProcedure, name_String}] :=
	Module[ {newTarget},
		...
		SymbolicCGenerate[cfun, name, "CodeTarget" -> newTarget, "CallerData" -> objData]
	]
...
getCompileDLLHeader[objData_, RTL_] :=
	Module[{funcData},
		...
		Flatten[{
			If[ RTL, 
				CDeclare[ {"static", "WolframCompileLibrary_Functions"}, "funStructCompile"]
			(* else *),
				{
				CInclude[ "WolframCompileLibrary.h"],
				CDeclare[ {"static", "WolframCompileLibrary_Functions"}, "funStructCompile"]
				}
			]
			,
...
Module[{args},
	args = {{"WolframLibraryData", "libData"}};
	CProgram[
		CInclude["WolframLibrary.h"],
		CFunction[{"EXTERN_C", "DLLEXPORT", "int"}, InitializeFunctionName[objData, name], args],
		CFunction[{"EXTERN_C", "DLLEXPORT", "void"}, UninitializeFunctionName[objData, name], args],
		fun
	]	
]
...
getDLLFunction[ objData_, name_] :=
	CFunction[ {InitExportString[objData], "int"}, "WolframLibrary_initialize", 
		...
		CBlock[
			{
			CReturn[CCall[InitializeFunctionName[objData, name], {"libData"}]]
			}
		]]
...
getNameFunction[objData_, name_] :=
	CFunction[{"DLLEXPORT", "char *"}, "WolframCompileLibrary_exportName", {},
		CBlock[{
...
(* else *),
	Module[{cfstring = ToString[FullForm[objData[Wrapper]]], code, res},
		...
		CFunction[{"DLLEXPORT", "char *"}, "WolframCompileLibrary_wrapper", {}, CBlock[code]]
	]
]
...
Module[{nargs = Length[objData[ArgumentTypes]], types, ranks, rest},
	...
	CFunction[{"DLLEXPORT", "mint"}, "WolframCompileLibrary_getArgumentTypes", {
			{"int **", "pt"},
			{"mint **", "pr"}
		},
		CBlock[{
...
Module[{funcData = objData[FunctionData], errlab = objData[ErrorLabel], body},
	...
	body = CIf[initializeVar,
		CBlock[{
			{CAssign["funStructCompile", CPointerMember["libData", "compileLibraryFunctions"]]},
			funcData[ConstantInitializationCode],
			If[funcData[FunctionPointerCount] > 0,
				Table[
					{
...
callUnaryMathFunction[objData_, "Reciprocal", out:Register[Real, _], arg:Register[Real, _], aflag_ /; Not[CheckOverflowQ[aflag]]] := 
	CAssign[ makeAtom[out, objData], COperator[ Divide, {1, makeAtom[arg, objData]}]]
	
callUnaryMathFunction[objData_, Square, out:Register[Integer, _], arg:Register[Integer, _], aflag_ /; Not[CheckIntegerOverflowQ[aflag]]] := 
	CAssign[ makeAtom[out, objData], COperator[Times, {makeAtom[arg, objData], makeAtom[arg, objData]}]]

callUnaryMathFunction[objData_, Square, out:Register[Real, _], arg:Register[Real, _], aflag_ /; Not[CheckOverflowQ[aflag]]] := 
	CAssign[ makeAtom[out, objData], COperator[Times, {makeAtom[arg, objData], makeAtom[arg, objData]}]]
	
callUnaryMathFunction[objData_, Square, out:Register[Complex, _], arg:Register[Complex, _], aflag_ /; Not[CheckOverflowQ[aflag]]] := 
...
callUnaryMathFunction[objData_, fun_?CStandardMathQ, out:Register[Real, _], arg:Register[Real, _], aflag_ /; Not[CheckOverflowQ[aflag]]] := 
...
callBinaryMathFunction[objData_, Power, out:Register[Real, _], {arg1:Register[Real, _], arg2:Register[Real, _]}, aflag_ /; Not[CheckOverflowQ[aflag]]] := 
...
callBinaryMathFunction[objData_, Power, out:Register[type:Real, _], {arg1:Register[type_, _], arg2:Register[Integer, _]}, aflag_ /; Not[CheckOverflowQ[aflag]]] := 
...
callBinaryMathFunction[objData_, op:(Plus | Subtract), out:Register[Complex, _], {arg1:Register[Complex, _], arg2:Register[Complex, _]}, aflag_ /; Not[CheckOverflowQ[aflag]]] := 
...
callBinaryMathFunction[objData_, Times, out:Register[Complex, _], 
	{arg1:Register[Complex, _], arg2:Register[Complex, _]}, aflag_ /; Not[CheckOverflowQ[aflag]]] := 
...
(*
	n-ary MTensor functions
*)
Apply[
	Function[MTensorNAryCode[#1] = #2],  
...
MTensorNAryQ[fun_] := And[MatchQ[MTensorNAryCode[fun], _Integer], SameQ[fun, ArgNName[MTensorNAryCode[fun]]]];

callMTensorNAryFunction[objData_, fun_, out_, args_] := 
Module[{n = Length[args], tar = makeAtom[Temporary[0], objData]},
	CBlock[
		CDeclare[typeToCType[MTensor], CArray[tar, n]],
		Table[CAssign[CArray[tar, i - 1], args[[i]]], {i, n}],
		CCall[objData[structCompile["MTensor_nArg"]], {MTensorNAryCode[fun], n, tar, out}]
	]
]
...
(*
	Basic MTensor properties 
*)
buildLine[num_, objData_, Instruction[Length, out_, arg:Register[MTensor[_, rank_],_]]] :=
	CAssign[makeAtom[out, objData], CArray[getDataDimensions[objData, arg],  0]]

buildLine[num_, objData_, Instruction[Dimensions, out_, {arg:Register[MTensor[type_, rank_],_], nreg_}]] :=
...
CBlock[
	...
	CheckError[objData, 
		CCall[objData[structCompile["MTensor_allocate"]], 
...
(* Build a MTensor from a constant list *)
buildLine[ num_, objData_, Instruction[Set, oreg:Register[MTensor[type_, rank_], no_], ConstantTensor[ct_List]]] :=
...
buildLine[ num_, objData_, Instruction[Set, out_, inp_]] :=
	Which[
		isTensor[out],
			{
				CheckError[objData, 
					CCall[objData[structCompile["MTensor_copyUnique"]], 
...
*)
buildLine[num_, objData_, Instruction["To0Rank", out:Register[MTensor[type_, 0], _], scalar_]] :=
{
	CheckError[objData, 
		CCall[objData[structCompile["MTensor_allocate"]], 
...
(* Build a MTensor from scalar data in registers *)
buildLine[ num_, objData_, Instruction[List, ldims_List, regout:Register[MTensor[type_, rank_], _], regElems_List]] /;
	(rank == Length[ldims]) :=
Module[{dims = makeAtom[Temporary[0], objData], out = makeAtom[regout, objData]},
CBlock[
	CDeclare[typeToCType[Integer], CAssign[CArray[dims, rank], ldims]],
	CheckError[objData, 
		CCall[objData[structCompile["MTensor_allocate"]], 
			{out, ToTypeEnum[type], rank, dims}]
	],
...
(* Build a MTensor from tensor data in registers *)		
buildLine[ num_, objData_, Instruction[List, ldims_List, regout:Register[MTensor[type_, rank_], _], regElems_List]] :=
Module[{dims = makeAtom[Temporary[0], objData], tens = makeAtom[Temporary[1], objData], i = makeAtom[Temporary[2], objData], dimsinit = Join[ldims, ConstantArray[0,{rank - Length[ldims]}]], olddims = getDataDimensions[objData, regElems[[1]]],  
	     out = makeAtom[regout, objData], setfun = objData[structCompile["MTensor_fillFromMTensor"]]},
CBlock[
	CDeclare[typeToCType[Integer], CAssign[CArray[dims, rank], dimsinit]],
	CDeclare[typeToCType[MTensor], CArray[tens, Length[regElems]]],
	CDeclare[typeToCType[Integer], i],
	CFor[CAssign[i, 0], LessC[ i, rank - Length[ldims]], COperator[ Increment, i], 
		CAssign[CArray[dims, COperator[Plus, {i, Length[ldims]}] ], CArray[olddims, i]]
	],
	CheckError[objData, 
		CCall[objData[structCompile["MTensor_allocate"]], 
			{out, ToTypeEnum[type], rank, dims}]
	...
	setDataPointer[objData, regout],
	Table[CAssign[CArray[tens, makeAtom[k - 1, objData]], makeTDAtom[regElems[[k]], objData]],
		{k, Length[regElems]}
	],
	CheckError[objData, CCall[setfun, {CDereference[out], tens}]]
]]
...
getElementGetFunction[ type_, rank_] :=
	Which[
		rank > 1,   "MTensor_GetElementMTensor",
		type == IntegerEnumType, "MTensor_GetElementInt",
		type == RealEnumType, "MTensor_GetElementReal",
		True, "MTensor_GetElementComplex"]
...
PartTypeRegister[{reg:Register[MTensor[_, 1], _], List}] := {1, reg}
PartTypeRegister[All] := {2, 0};
PartTypeRegister[{reg:Register[MTensor[_, 1], _], Span}] := {3, reg}
...
getElementFunction[objData_, type_, rank_, depth_] := 
	If[rank == 0,
		objData[struct["MTensor_get" <> ToString[type]]]
	(* else *),
		objData[structCompile["MTensor_getMTensorInitialized"]]
	]

setElementFunction[objData_, type_, rank_, depth_] := 
	If[rank == 0,
		objData[struct["MTensor_set" <> ToString[type]]]
	(* else *),
		objData[struct["MTensor_setMTensor"]]
	]
...
Flatten[{
	...
	(* else *)
		function = objData[structCompile[If[setQ, "MTensor_setPart", "MTensor_getPart"]]];
		callWithCollectedSpecifications[objData, function, res, arg, pspecs]
	]
}]
]

buildLine[num_,objData_, Instruction["GetElement", res_, {arg:Register[MTensor[type_, argrank_], _], inds__}]] := 
		...
		CBlock[
			If[rank == 0,
			{
				...
				Table[CAssign[CArray[pos, i - 1], indexC[i]],
					{i, depth}],
				CCall[objData[structCompile["MTensor_getMTensorInitialized"]], {makeAtomAddress[res, objData], makeTDAtom[arg, objData], pos, depth}],
				setDataPointer[objData, res]
			}]
		]
	]
...
buildLine[ num_, objData_, Instruction["SetElement", regTen_, regPos_, regValue_]] :=
	Module[ {rank, type, pos},
		...
		If[isTensor[regValue],
			CheckError[objData, 
				CCall[objData[structCompile["MTensor_insertMTensor"]], 
...
buildLine[num_, objData_, Instruction[fun_?ComparisonQ, out_, {tol_, argseq:(Register[MTensor[_, _], _]..)}]] :=
Module[{pargs, n, cenum = ComparisonEnum[fun], args = {argseq}},
	pargs = makeAtom[Temporary[0], objData];
	n = Length[args];
	CBlock[
		CDeclare[typeToCType[MTensor], CArray[pargs, n]],
...
Module[{typeout = convertFullType[out], typeargs = Map[convertFullType,args], aflag = objData[ArithmeticFlags]},
	If[CBasicTypeQ[typeout] && 
			(Not[CheckIntegerOverflowQ[aflag]] && (Union[typeargs[[All,1]]] === {Integer})) ||
	   		(UnsameQ[First[typeout], Integer] && Not[CheckOverflowQ[aflag]] && Apply[And, Map[CBasicTypeQ, typeargs]]),
	...
	Module[ {name, cnt, funcData, numArgs, args, assignArgs, resArg, call, comment, convert, c = 0, r, f, afun, flowcheck = False},
		...
		If[ !StringQ[name]
			...
			If[objData[WolframRTL] && (Head[fun] === LibraryFunction), 
				Message[CCodeGenerate::wmreq, fun]
			];
			...
			funcData[FunctionPointerType][cnt] = "LibraryFunctionPointer";
		...
		If[Head[fun] === LibraryFunction, 
			...
			flowcheck = (CheckOverflowQ[objData[ArithmeticFlags]] && MatchQ[getType[out], Real | Complex]);
		];
		numArgs = Length[ regs];
		...
		convert = Flatten[Last[Reap[
			assignArgs =
					Table[
						r = regs[[i]];
						afun = MArgumentGetAddressFunction[r];
						If[RankZeroTensorQ[r],
							f = r /. MTensor[type_, 0]->type;
							r = Temporary[c];
							Sow[
								CDeclare[
									typeToCType[MTensor], 
									CAssign[
										makeAtom[r, objData], 
										CCall[
											objData[structCompile["getRankZeroMTensor"]], 
		...
		afun = MArgumentGetAddressFunction[out];
		...
		call =
		  If[(Head[fun] === LibraryFunction) && ReturnsTensor[fun],
			Module[{oat = makeTDAtom[out, objData], temp = "Temp"}, CBlock[{
				CDeclare[typeToCType[MTensor], CAssign[ temp, oat]],
				...
				CIf[COperator[Unequal, {temp, oat}],
					CBlock[{
						CCall[objData[structCompile["MTensor_copy"]], {makeAtom[out, objData], temp}],
						CCall[objData[struct["MTensor_free"]], {temp}]
					}]
				]
			}]]
		, (*else *)
...
MakeRegisterType[{type_, rank_?Positive, ___}] := MTensor[type, rank];
...
TypeRankReference[reg:Register[MTensor[type_, rank_], _], objData_] := {type, rank, VoidPointer[makeAtomAddress[reg, objData]]}
...
AssignTypeReference[objData_, reg:Register[MTensor[__], _], i_, tvar_, ref_] := {CAssign[CArray[tvar, i], 0], CAssign[CArray[ref, i], VoidPointer[makeAtomAddress[reg, objData]]]}
...
Module[{name, code, funcData, args, flowcheck = 0, aflags = objData[ArithmeticFlags]},
	If[(CheckOverflowQ[objData[ArithmeticFlags]] && MatchQ[getType[out], Real | Complex]), 
	...
	If[!StringQ[name]
		...
		code = SymbolicCGenerate[cproc, name, "CodeTarget"->"NestedFunction", "CallerData"->objData];
...
(
	...
	If[TrueQ[$IssueGeneric],
		...
		Throw[Message[CCodeGenerate::nosupp, inst]; $Failed, "Not Implemented"]
	]
)
