...
BeginPackage["CompileAST`Export`Format`CodeFormatter`"]
...
$alwaysBreakCompoundExpressions = True;
...
$blocks = {
   ...
   GeneralBlock, ParenBlock, InlineListBlock, ListBlock, PatternBlock, PatternTestBlock, 
   FunctionBlock, FullFunctionBlock, AlternativesBlock, StatementBlock, NewlineBlock, 
...
postformat[FullFunctionBlock[vars_, body___]] :=
    RowBox[{"Function", "[", postformat@vars,
      RowBox[{Sequence @@ (Map[postformat, {body}] //. 
         EmptySymbol[] :> Sequence[])}],
      "]"}];
...
postformat[InlineListBlock[elems___]] :=
    RowBox[{"{", 
...
format[expr : GeneralHeadBlock[head:"Function", vars_, elems___], currentTab_] :=
    With[ {},
        FullFunctionBlock[
          format[vars /. ListBlock -> InlineListBlock, currentTab],
          Sequence @@ Map[
            format[TabBlock@NewlineBlock[#, False], currentTab] &,
            {elems}],
...
format[expr : (InlineListBlock[elems___]), currentTab_] /; needSplitQ[expr, currentTab] :=
    ListBlock[
      Sequence @@ Map[format[#, currentTab] &, {elems}],
      closingElement["List"]
      ];
...
makeBoxes[HoldForm[e_]] := makeBoxes[e]
