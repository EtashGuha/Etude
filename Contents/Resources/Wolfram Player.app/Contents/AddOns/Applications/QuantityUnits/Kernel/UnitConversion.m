...
c2sibu[q:Quantity[mag_, unit_]] := Block[{IUnitEnum, IUnitIt = 0}, Module[{qu1, qu2, num, den},
	...
	qu2 = With[{u = PowerExpand@Divide[Times@@qu1, Times@@qu2]}, Quantity[PowerExpand[num/den /. InertInterval -> Interval], u]];
...
iUnitConvert[s_String] /; Not[KnownUnitQ[s]] := With[{q = Quantity[s]}, If[Quiet[QuantityQ[q]], iUnitConvert[q, "Base"], Throw[False, "iUnitConvert"]]]
...
iUnitConvert[(quantity_Quantity)?QuantityQ,"Base"] := Module[{q = quantity},
	If[MixedUnitQ[quantity], q = unmixMixedUnitQuantity[quantity]];
	c2sibu[q]
]

iUnitConvert[quantities:{_Quantity?QuantityQ..}, "Base"] := UnitConvert[#, "Base"]& /@ quantities
...
getUnitSystemList[u_String] := With[{a = QuantityUnits`$UnitTable[u]},
	If[
		MissingQ[a],
		getUnitSystemList[fromUnitAlias[u]],
		a["UnitSystem"]
	]
]

getPrimaryDestX[u_String] := ReplaceAll[With[{a = QuantityUnits`$UnitTable[u]},
	If[
		MissingQ[a],
		getPrimaryDestX[fromUnitAlias[u]],
		a["UnitPrimaryDestinationsX"]
	]
], {"SIPrefixedList"[args__] :> Sequence[args], "MixedRadixUnit"[___] :> Nothing}]

unitSystemMember[system_, Quantity[_, unit_String]] := unitSystemMember[system, unit]
...
selectBestFitQuantity[q_Quantity, l_List] := Module[{conversion = UnitConvert[q,#]& /@ l},
	conversion = Cases[conversion, _?normallyBoundedQuantityQ];
...
iUnitConvert[n_?NumericQ, targ_?KnownUnitQ] := If[UnitDimensions[targ] === {},
	With[{value = iUnitConvert[targ, "Base"]}, Quantity[n / value, targ]], 
	Message[Quantity::compat, "DimensionlessUnit", targ]; Throw[False, "iUnitConvert"]
	]
iUnitConvert[n_?NumericQ, targ_?QuantityQ] := If[UnitDimensions[targ] === {},
	With[{value = iUnitConvert[targ, "Base"], unit = QuantityUnit[targ]}, Quantity[n / value, unit]],
	Message[Quantity::compat, "DimensionlessUnit", QuantityUnit[targ]]; Throw[False, "iUnitConvert"]
	]
iUnitConvert[(q_Quantity)?QuantityQ, n_?NumericQ] /; UnitDimensions[q] === {} := QuantityExpand[q]
iUnitConvert[(q_Quantity)?QuantityQ, n_?NumericQ] := (Message[Quantity::compat, "DimensionlessUnit", QuantityUnit[q]]; Throw[False, "iUnitConvert"])
...
iUnitConvert[q:Quantity[_, unit_], target_] /; Not[CompatibleUnitQ[unit, target]] := Module[{iud = UnitDimensions[unit], ud = UnitDimensions[target], tu},
	If[
		And[
			MatchQ[iud, {__List}],
			MatchQ[ud, {__List}],
...
(*fromUnitAlias is in QuantityUnits.m*)

unitTableFValueLookup[u_String] := With[{a = QuantityUnits`$UnitTable[u]},
	If[
		MissingQ[a],
		unitTableFValueLookup[fromUnitAlias[u]],
		a["FundamentalUnitValue"]
	]
]
...
unitTableFZValueLookup[u_String] := With[{a = QuantityUnits`$UnitTable[u]},
	If[MissingQ[a],
		unitTableFZValueLookup[fromUnitAlias[u]], 
		a["FundamentalZeroValue"]
	]
]
...
Scan[
	(
		CUF0R[#] := CUF0R[#] = (CUF0[#] /. f_Function :> f[1]);
		CUF1R[#] := CUF1R[#] = (CUF1[#] /. f_Function :> f[1]);
	) &,
	QuantityUnits`$UnitList
]
...
QuantityPrimaryDestinationFunction[Quantity[_,unit_]?QuantityQ]:=UnitPrimaryDestinations[u]
