...
200)loadOptions.streaming=true;me.loadDocument(doc,loadOptions)},function(){loadOptions.streaming=true;me.loadDocument(doc,loadOptions)})}if(me.instance.docViewer.getDocument()===null)viewerWindow.$(iframe.contentDocument).on("documentLoaded",function(event){me._trigger(event.type)});else me._trigger("documentLoaded");viewerWindow.$(iframe.contentDocument).on("displayModeChanged layoutModeChanged zoomChanged pageChanged fitModeChanged toolModeChanged printProgressChanged error",function(){var event=
arguments[0];me._trigger(event.type,Array.prototype.slice.call(arguments,1))})}catch(error){console.warn("Viewer is on a different domain, API functions may not work because of cross domain permissions.")}},_isPDFExtension:function(extension){var result=false;if(extension){var pdfExtensions=[".pdf",".png",".jpg","jpeg"];pdfExtensions.forEach(function(ext){if(extension===ext)result=true})}return result},_isOfficeExtension:function(extension){var result=false;if(extension){var officeExtensions=[".docx",
".doc",".pptx"];officeExtensions.forEach(function(ext){if(extension===ext)result=true})}return result},_getHTML5OptionsURL:function(){if(this.selectedType==="html5"){if(this.options.html5Options)Utils.extend(this.options,this.options.html5Options)}else if(this.selectedType==="html5Mobile")if(this.options.html5MobileOptions)Utils.extend(this.options,this.options.html5MobileOptions);var options=this.options;var url="";function acceptBackendUrl(officeEnabled,pdfEnabled){if(officeEnabled)url+=options.backendType?
"&office="+options.backendType:"&office=auto";if(pdfEnabled)url+=options.backendType?"&pdf="+options.backendType:"&pdf=auto"}if(options.initialDoc)url+="#d="+options.initialDoc;var officeEnabled=false,pdfEnabled=false;var periodPosition;var extension=options.initialDoc&&(periodPosition=options.initialDoc.lastIndexOf("."))?options.initialDoc.slice(periodPosition).toLowerCase():null;if(options.documentType==="pdf"||options.documentType==="all"||typeof options.documentType==="undefined"&&this._isPDFExtension(extension))pdfEnabled=
true;if(options.documentType==="office"||options.documentType==="all"||typeof options.documentType==="undefined"&&this._isOfficeExtension(extension))officeEnabled=true;if(options.documentType)url+="&documentType="+options.documentType;if(typeof options.backendType==="undefined")options.backendType=options.pdfBackend;acceptBackendUrl(officeEnabled,pdfEnabled);if(options.streaming)url+="&streaming="+options.streaming;if(options.externalPath){var path=this._correctRelativePath(options.externalPath);
path=encodeURIComponent(path);url+="&p="+path}if(options.encryption)url+="&auto_load=false";if(options.enableAnnotations)url+="&a=1";if(options.disabledElements&&options.ui!=="legacy"){var disabledElements=encodeURIComponent(options.disabledElements.join(","));url+="&disabledElements="+disabledElements}if(options.serverUrl){var serverUrl=this._correctRelativePath(options.serverUrl);serverUrl=encodeURIComponent(serverUrl);url+="&server_url="+serverUrl}if(options.serverUrlHeaders)url+="&serverUrlHeaders="+
JSON.stringify(options.serverUrlHeaders);if(options.documentId)url+="&did="+options.documentId;if(options.css){var css=this._correctRelativePath(options.css);css=encodeURIComponent(css);url+="&css="+css}if(options.config){var config=this._correctRelativePath(options.config);config=encodeURIComponent(config);url+="&config="+config}if(options.disableI18n&&options.ui!=="legacy")url+="&disableI18n=1";if(options.enableOfflineMode)url+="&offline=1";if(options.startOffline)url+="&startOffline=1";if(options.enableReadOnlyMode)url+=
...
"&preloadWorker="+(options.preloadPDFWorker?1:0);if(typeof options.fullAPI!=="undefined")url+="&pdfnet="+(options.fullAPI?1:0);if(typeof options.showToolbarControl!=="undefined")url+="&toolbar="+(options.showToolbarControl?"true":"false");if(typeof options.showPageHistoryButtons!=="undefined")url+="&pageHistory="+(options.showPageHistoryButtons?1:0);if(typeof options.xdomainProxyUrl!=="undefined"){var urls;if(typeof options.xdomainProxyUrl==="string")urls={url:options.xdomainProxyUrl};else urls=options.xdomainProxyUrl;
