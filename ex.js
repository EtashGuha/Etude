...
for (var i = textArray.length - 1; i >= 0; i--) {
	...
	if (noMatchList.length != 0) {
		clessq.forEach((item) => {
			// console.log(stringSimilarity.findBestMatch(item, noMatchList).bestMatch)
			var bestMatch = stringSimilarity.findBestMatch(item, noMatchList).bestMatch
			sumOfMatches += bestMatch.rating
			// noMatchList.splice(noMatchList.indexOf('foo'), 1)
			// .splice( list.indexOf('foo'), 1 );
		})
	}
	console.log(sumOfMatches/clessq.size)
	var rating = (sharedSize + (sumOfMatches/clessq.size)) / (question.size)
	console.log(textArray[i])
	console.log(rating)
	if(Number.isNaN(rating)){
		console.log("NOPEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE")
		continue;
	}
	if(minHeap.size() <= 8) {
